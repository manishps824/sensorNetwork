    ...
    if( XBee.available() )
    {
	 ...
	 speedString = strtok(NULL, "#");   //sec_part points to "name"
        ...
        speed = atoi(speedString);
         ...
         xbee802.setDestinationParams(newPaq_sent, "0013A2004086A494", copyMsg, MAC_TYPE, DATA_ABSOLUTE);        
        ...
        while(millis() < temp)
         {
          ...
          // XBee.println(millis());
          sprintf(data,"W01#ACCX#%d#ACCY#%d#ACCZ#%d%c#%c#%s%c\n",ACC.getX(), ACC.getY(), ACC.getZ(), '\r','1', copyMsg,'\n'); 
           xbee802.setDestinationParams(paq_sent, mac, data, MAC_TYPE, DATA_ABSOLUTE);
